...
export class AxiosService implements OnModuleInit {
	...
	constructor(private readonly configService: TypedConfigService) {
		...
		if (
			this.configService
				.getOrThrow('REMNAWAVE_PANEL_URL')
				.startsWith('http://')
		) {
			this.axiosInstance.defaults.headers.common['X-Forwarded-For'] =
				'127.0.0.1'
			this.axiosInstance.defaults.headers.common['X-Forwarded-Proto'] =
				'https'
		}
	...
	async onModuleInit(): Promise<void> {
		...
		this.logger.log(
			`Remnawave API URL: ${this.axiosInstance.defaults.baseURL}`,
		)
		...
		if (!remnawaveMetadata.isOk || !remnawaveMetadata.remnawaveVersion) {
			...
			this.logger.log(
				`[OK] Connected to Remnawave v${remnawaveMetadata.remnawaveVersion}`,
			)
		}
	}
	...
	public async getRemnawaveMetadata(): Promise<{
		...
		try {
			const response =
				await this.axiosInstance.request<GetMetadataCommand.Response>({
				...
				await this.axiosInstance.request<GetUserByUsernameCommand.Response>(
					{
						method: GetUserByUsernameCommand.endpointDetails
							.REQUEST_METHOD,
						url: GetUserByUsernameCommand.url(
	...
	public async getSubscriptionPageConfigByUuid(
		...
		ICommandResponse<GetSubscriptionPageConfigCommand.Response['response']>
	> {
		try {
			const response =
				await this.axiosInstance.request<GetSubscriptionPageConfigCommand.Response>(
					{
						method: GetSubscriptionPageConfigCommand.endpointDetails
							.REQUEST_METHOD,
						url: GetSubscriptionPageConfigCommand.url(
			...
			this.logger.error(
				'Error in GetSubscriptionPageConfigByUuid Request:',
				error,
			)
		...
		try {
			const response =
				await this.axiosInstance.request<GetSubscriptionPageConfigsCommand.Response>(
					{
						method: GetSubscriptionPageConfigsCommand
			...
			const validationResult =
				await GetSubscriptionPageConfigsCommand.ResponseSchema.parseAsync(
					response.data,
				)
			...
			if (error instanceof AxiosError) {
				...
				this.logger.error(
					`Subpage Config List Request failed: ${error.message}`,
				)
				return { isOk: false }
			} else {
				this.logger.error(
					`Subpage Config List Request failed: ${error}`,
				)
				return { isOk: false }
			}
		}
	...
	public async getSubscriptionInfo(
		...
		ICommandResponse<GetSubscriptionInfoByShortUuidCommand.Response>
	> {
		try {
			const response =
				await this.axiosInstance.request<GetSubscriptionInfoByShortUuidCommand.Response>(
					{
						method: GetSubscriptionInfoByShortUuidCommand
							.endpointDetails.REQUEST_METHOD,
						url: GetSubscriptionInfoByShortUuidCommand.url(
			...
			if (error instanceof AxiosError) {
				this.logger.error(
					'Error in GetSubscriptionInfo Request:',
					error.message,
				)
			} else {
				this.logger.error(
		...
		try {
			const response =
				await this.axiosInstance.request<GetSubpageConfigByShortUuidCommand.Response>(
					{
						method: GetSubpageConfigByShortUuidCommand
							.endpointDetails.REQUEST_METHOD,
						url: GetSubpageConfigByShortUuidCommand.url(
							encodeURIComponent(shortUuid),
						),
						data: {
							requestHeaders,
	...
	public async getSubscription(
		...
		try {
			...
			if (error instanceof AxiosError) {
				...
				this.logger.error(
					`Error in GetSubscription Request: ${error.message}`,
				)
			} else {
